...
Using [Keyboard Maestro](https://www.keyboardmaestro.com/), switching environments has been as quick as a single <kbd>Ctrl+E</kbd> keyboard shortcut to toggle environment URLs.
...
1. **Store in a variable**: Chrome's current URL gets stored in a variable called `TheURL`.
2. **Switch logic**: A switch/case block checks the URL and replaces current environment with the alternate environment.
3. **Navigate to new URL**: The modified URL is pasted back in Chrome's address bar and Enter is pressed.
...
And that's it. Keyboard Maestro is the best (as of now) $36 I've ever spent in my life.

This example switches between Cloudflare documentation environments, you can customize the macro for anything else by changing the URL strings in the switch statement. Small automations like this compound into huge workflow improvements when used every day without even thinking about them.