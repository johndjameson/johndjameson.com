...
What’s [Cloudflare R2](https://www.cloudflare.com/developer-platform/r2/)? If you’re familiar with Amazon S3 for storing static files and media, Cloudflare sells their R2 service as S3-compatible object storage minus any bandwidth fees to access the files once uploaded (AKA “egress”). For my purposes, their [pricing calculator](https://r2-calculator.cloudflare.com/) says it’s $0.00. I enjoy using other Cloudflare products, so I decided to give R2 a try!

If like me, you want to buy a new domain and set up an R2 bucket for hosting web files, here are the steps to make that happen:
